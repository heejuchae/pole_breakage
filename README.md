...
정리하면, 이 단계는 **“파단이 어디에 어떻게 나타나는가”를 먼저 배우는 단계**이며,

또한 confidence 출력 구조는 포함되어 있지만, 현재 설정에서는 **실질적으로 bbox 위치 학습이 중심**이며, confidence 항은 주요 학습 목표로 적극 활용하지 않는 설정입니다.
...
최종 모델 가중치 파일은 `final_binary.keras`입니다.

축별 특징을 결합한 **최종 분류 모델 하나**를 저장합니다.
...
- **FN example**: 실제 라벨은 Break이지만 최종적으로 Normal로 분류된 사례입니다. 이 샘플은 **x축에만 유효한 bbox가 존재하고, y축과 z축에는 bbox가 없었습니다.** 즉, 다른 축에서 파단을 뒷받침하는 신호가 부족한 상태였고, 그 결과 최종 확률이 threshold를 약간 넘지 못해 FN으로 분류되었습니다.
